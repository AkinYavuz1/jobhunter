...
export async function searchTotalJobs(term: string): Promise<RawJob[]> {
  ...
  try {
    for (let page = 1; page <= 3; page++) {
      ...
      const url = `https://www.totaljobs.com/jobs/${encodedTerm.replace(/%20/g, '-')}/in-united-kingdom?radius=0&postedWithin=7&contractType=permanent&remote=true&page=${page}`;
...
function parseSalaryText(text: string): { min: number | null; max: number | null } {
  ...
  const factor1 = matches[0].toLowerCase().includes('k') ? 1000 : 1;
  const factor2 = matches[0].toLowerCase().includes('k') ? 1000 : 1;
  return {
    min: Math.round(parseFloat(matches[1]) * factor1),
    max: Math.round(parseFloat(matches[2]) * factor2),
  };
}
